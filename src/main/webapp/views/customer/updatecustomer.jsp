...
<form class="layui-form "  lay-filter="customer-form"> 
	...
	<div class="layui-form-item">
		<div class="layui-input-block">
			<button type="button" name="customer-form-submit-btn" lay-submit lay-filter="customer-form-submit" class="layui-btn" style="margin-left: 80%;">保存</button>
		</div>
	</div>	
...
layui.use(['form','upload'],function(){
	...
	form.on('submit(customer-form-submit)',function(data){
		...
		//添加客户时检验客户名可用性
		if($('#customer-name-msg').text() == '客户名已存在' && $('input[name=name]').val() != name){
    ...
    $('input[name=name]').blur(function(){
    	...
    	if(newname == name){
    		$('#customer-name-msg').text('');
    		return ;
    	}
        $.post('${pageContext.request.contextPath}/customer/checkname',{'name':newname},function(data){
        	if(data){
               ...
               if(data.success){//成功
                var customer = data.data;
               	...
               	if(customer.product != null){
               		$('#choose-product-name').text(customer.product.name);
               	}
